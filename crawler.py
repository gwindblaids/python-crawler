...
# регулярное выражение для поиска ссылок на домен .onion в тексте страниц (не среди html-тегов)
...
NUMBER_OF_THREADS = 50  # количество потоков (количество ссылок обрабатываемых программой) можно увеличить до 50, больше могут быть ошибки
...
FILE_LINKS_FROM_TEXT = 'links_from_text.txt' # сюда сохраняются ссылки найденные в тексте страниц, а не среди html-тегов.
#  После проверки на правильность ссылки из этого файла можно добавить в файл all_links.csv
...
def connection_tor():
    ...
    Этот код нужен для подключения к сети tor
    """
    socks.set_default_proxy(socks.SOCKS5, "127.0.0.1", 10123)
...
def main():
    ...
    file_name_list = ['all_links.csv', 'visited_links.csv', 'index_start.txt', 'error.csv', 'domains_counter.csv', 'links_from_text.txt']
